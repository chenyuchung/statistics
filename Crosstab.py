...
def crosstab(dataframe,indexName,columnName,weight=None):
    
    dfn = dataframe.reset_index(drop=True)

    if weight != None:
        dfc = pd.crosstab(dfn[indexName],dfn[columnName],dfn[weight],aggfunc = sum).reset_index()
    ...
    for i in yitems:
        ...
        dfc[str(int(i))+'%'] = dfc[i] / dfc[yitems].sum(axis=1)   #row_percentage
